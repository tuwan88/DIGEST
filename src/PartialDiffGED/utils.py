...
def load_all_graphs(data_location, dataset_name):
    ...
    if isfile(split_count_path):
        ...
        expected_train_total = train_num + val_num
        if expected_train_total != train_total:
            raise ValueError(
                "graph_split_counts.json mismatch for {}: train+val={} but train dir has {} graphs.".format(
                    dataset_name, expected_train_total, train_total
                )
            )
        if int(split_counts["test"]) != test_num:
            raise ValueError(
                "graph_split_counts.json mismatch for {}: test={} but test dir has {} graphs.".format(
                    dataset_name, int(split_counts["test"]), test_num
                )
            )
    else:
        val_num = test_num
...
def load_labels(data_location, dataset_name):
    ...
    train_onehot_paths = get_file_paths(data_location + "json_data/" + dataset_name + "/train", "onehot")
    test_onehot_paths = get_file_paths(data_location + "json_data/" + dataset_name + "/test", "onehot")
    ...
    graphs = iterate_get_graphs(data_location + "json_data/" + dataset_name + "/train", "json") \
           + iterate_get_graphs(data_location + "json_data/" + dataset_name + "/test", "json")
    ...
    print('Load label-derived one-hot features (dim = {}) of {}.'.format(feature_dim, dataset_name))
    return global_labels, features
