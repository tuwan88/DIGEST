...
static void print_matching(unsigned long long pair_idx, const Graph *db_graph, const Graph *query_graph,
		const vector<pair<ui,ui> > &mapping, bool q_g_swapped) {
	const Graph *internal_query_graph = q_g_swapped ? db_graph : query_graph;
	const Graph *internal_db_graph = q_g_swapped ? query_graph : db_graph;
	printf("matching %llu (%s, %s): {\"query_graph_id\":\"%s\",\"db_graph_id\":\"%s\",\"q_g_swapped\":%s,\"query_to_db\":[",
		pair_idx,
		db_graph->id.c_str(),
		query_graph->id.c_str(),
		json_escape(internal_query_graph->id).c_str(),
		json_escape(internal_db_graph->id).c_str(),
		q_g_swapped ? "true" : "false");
	for(ui i = 0;i < mapping.size();i ++) {
		if(i) printf(",");
		printf("[%u,%u]", mapping[i].first, mapping[i].second);
	}
	printf("]}\n");
}

void print_usage() {
	printf("Usage: ./ged -h -d database_file -q query_file -m running_mode -p search_paradigm -l lower_bound_method -t ged_threshold -k stop_states\n");
...
int main(int argc, char *argv[]) {
	...
	bool print_matching_flag = false;
	...
	auto stop_option = op.add<Value<int>>("k", "stop", "\'stop the search after k state expansions; if not provided, then traverse the whole search space", -1, &stop);
	op.add<Switch>("g", "ged", "\'print_ged\'", &print_ged);
	op.add<Switch>("x", "matching", "\'print_matching\'", &print_matching_flag);
	...
	if(strcmp(mode.c_str(), "pair") == 0) {
		...
		for(ui i = 0;i < queries.size();i ++) {
			...
			if(res <= verify_upper_bound) ++ results_cnt;
				...
				if(print_matching_flag && res != -1) {
					vector<pair<ui,ui> > mapping;
					app->get_mapping(mapping, app->is_q_g_swapped() ? db[i]->n : queries[i]->n);
					print_matching(i, db[i], queries[i], mapping, app->is_q_g_swapped());
				}
		...
		for(ui i = 0;i < queries.size();i ++) {
			for(ui j = 0; j < db.size();j ++) {
				...
				assert(res == app->compute_ged_of_BX());
					...
					if(print_matching_flag) {
						vector<pair<ui,ui> > mapping;
						app->get_mapping(mapping, app->is_q_g_swapped() ? db[j]->n : queries[i]->n);
						print_matching(i*(unsigned long long)(db.size())+j, db[j], queries[i], mapping, app->is_q_g_swapped());
					}
					total_res += res;
